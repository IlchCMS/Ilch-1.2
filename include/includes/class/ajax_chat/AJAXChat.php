...
class AJAXChat {
	...
	function initConfig() {
		$config = null;
		if (!include(AJAX_CHAT_PATH.'/include/includes/func/ajax_chat/config.php')) {
			echo('<strong>Error:</strong> Could not find a config.php file in "'.AJAX_CHAT_PATH.'/include/includes/func/ajax_chat/". Check to make sure the file exists.');
			die();
		}
		$this->_config = &$config;
	...
	function makeSafe($value,$type) {
		return "'".escape($value,$type)."'";
	}
	...
	function getTemplateFileName() {
		switch($this->getView()) {
			case 'chat':
				return AJAX_CHAT_PATH.'/include/templates/ajax_chat/loggedIn.html';
			case 'logs':
				return AJAX_CHAT_PATH.'/include/templates/ajax_chat/logs.html';
			default:
				return AJAX_CHAT_PATH.'/include/templates/ajax_chat/loggedOut.html';
		}
	}
	...
	function addToOnlineList() {
		$sql = 'INSERT INTO `prefix_ajax_chat_online` (
					userID,
					userName,
					userRole,
					channel,
				...
				VALUES (
					'.$this->makeSafe($this->getUserID(),'integer').',
					'.$this->makeSafe($this->getUserName(),'string').',
					'.$this->makeSafe($this->getUserRole(),'string').',
					'.$this->makeSafe($this->getChannel(),'string').',
					NOW(),
					'.$this->makeSafe($this->ipToStorageFormat($_SERVER['REMOTE_ADDR']),'string').'
				);';	
	...
	function removeFromOnlineList() {
		$sql = 'DELETE FROM
					`prefix_ajax_chat_online` 
				WHERE
					userID = '.$this->makeSafe($this->getUserID(),'integer').';';
	...
	function updateOnlineList() {
		$sql = 'UPDATE
					`prefix_ajax_chat_online` 
				SET
					userName 	= '.$this->makeSafe($this->getUserName(),'string').',
					channel 	= '.$this->makeSafe($this->getChannel(),'string').',
					dateTime 	= NOW(),
					ip			= '.$this->makeSafe($this->ipToStorageFormat($_SERVER['REMOTE_ADDR']),'string').'
				WHERE
					userID = '.$this->makeSafe($this->getUserID(),'integer').';';
	...
	function deleteMessage($messageID) {
		...
		$sql = 'SELECT
					channel
				FROM
					`prefix_ajax_chat_messages` 
				WHERE
					id='.$this->makeSafe($messageID,'integer').';';
		...
		if($row['channel'] !== null) {
			...
			if($this->getUserRole() == AJAX_CHAT_ADMIN) {
				...
				$condition = '	AND
									NOT (userRole='.$this->makeSafe(AJAX_CHAT_ADMIN,'string').')
								AND
									NOT (userRole='.$this->makeSafe(AJAX_CHAT_CHATBOT,'string').')';
				...
				$condition = 'AND
								...
								OR
									(
									channel = '.$this->makeSafe($this->getPrivateMessageID(),'integer').'
									OR
									channel = '.$this->makeSafe($this->getPrivateChannelID(),'integer').'
									)
									AND
										NOT (userRole='.$this->makeSafe(AJAX_CHAT_ADMIN,'string').')
									AND
	...
	function insertCustomMessage($userID, $userName, $userRole, $channelID, $text, $ip=null, $mode=0) {
		...
		$sql = 'INSERT INTO `prefix_ajax_chat_messages` (
								userID,
								userName,
								userRole,
								channel,
				...
				VALUES (
					'.$this->makeSafe($userID,'integer').',
					'.$this->makeSafe($userName,'string').',
					'.$this->makeSafe($userRole,'string').',
					'.$this->makeSafe($channelID,'integer').',
					NOW(),
					'.$this->makeSafe($this->ipToStorageFormat($ip),'integer').',
					'.$this->makeSafe($text,'textarea').'
				);';
	...
	function kickUser($userName, $banMinutes=null, $userID=null) {
		...
		$sql = 'DELETE FROM
					`prefix_ajax_chat_online` 
				WHERE
					userID = '.$this->makeSafe($userID,'integer').';';
	...
	function banUser($userName, $banMinutes=null, $userID=null) {
		...
		$sql = 'INSERT INTO `prefix_ajax_chat_bans` (
					userID,
					userName,
					dateTime,
					ip
				...
				VALUES (
					'.$this->makeSafe($userID,'integer').',
					'.$this->makeSafe($userName,'string').',
					DATE_ADD(NOW(), interval '.$this->makeSafe($banMinutes,'string').' MINUTE),
					'.$this->makeSafe($this->ipToStorageFormat($ip),'string').'
				);';	
	...
	function unbanUser($userName) {
		$sql = 'DELETE FROM
					`prefix_ajax_chat_bans` 
				WHERE
					userName = '.$this->makeSafe($userName,'string').';';
	...
	function setInactive($userID, $userName=null) {
		$condition = 'userID='.$this->makeSafe($userID,'integer');
		if($userName !== null) {
			$condition .= ' OR userName='.$this->makeSafe($userName,'string');
		}
		$sql = 'UPDATE
					`prefix_ajax_chat_online` 
				SET
	...
	function removeInactive() {
		...
		if(db_num_rows($result) > 0) {
			...
			while($row = db_fetch_assoc($result)) {
				...
				$condition .= 'userID='.$this->makeSafe($row['userID'],'integer');
	...
	function getMessageCondition() {
		$condition = 	'id > '.$this->makeSafe($this->getRequestVar('lastID'),'integer').'
						AND (
							channel = '.$this->makeSafe($this->getChannel(),'string').'
							OR
							channel = '.$this->makeSafe($this->getPrivateMessageID(),'integer').'
	...
	function getLogoutXMLMessage() {
		...
		$xml .= '<![CDATA['.$this->getConfig('logoutData').']]>';
		$xml .= '</info>';
		$xml .= '</infos>';
		$xml .= '</root>';
		return $xml;
	...
	function getTeaserMessageCondition() {
		...
		$condition = 	'channel = '.$this->makeSafe($channelID,'integer').'
						AND
						';
		if($this->getConfig('requestMessagesPriorChannelEnter') ||
	...
	function getLogsViewCondition() {
		...
		switch($this->getRequestVar('channelID')) {
			case '-3':
				...
				if($this->getUserRole() != AJAX_CHAT_ADMIN) {
					$condition .= ' AND (channel = '.$this->makeSafe($this->getPrivateMessageID(),'integer');
					$condition .= ' OR channel = '.$this->makeSafe($this->getPrivateChannelID(),'integer');
					foreach($this->getChannels() as $channel) {
						if($this->getConfig('logsUserAccessChannelList') && !in_array($channel, $this->getConfig('logsUserAccessChannelList'))) {
							continue;
						}
						$condition .= ' OR channel = '.$this->makeSafe($channel,'string');
					}
					$condition .= ')';
				}
				break;
			...
			default:
				if(($this->getUserRole() == AJAX_CHAT_ADMIN || !$this->getConfig('logsUserAccessChannelList') || in_array($this->getRequestVar('channelID'), $this->getConfig('logsUserAccessChannelList')))
					...
					$condition .= ' AND channel = '.$this->makeSafe($this->getRequestVar('channelID'),'integer');
				} else {
					// No valid channel:
					$condition .= ' AND 0 = 1';
				}
		...
		if($this->getRequestVar('search')) {
			if(($this->getUserRole() == AJAX_CHAT_ADMIN || $this->getUserRole() == AJAX_CHAT_MODERATOR) && strpos($this->getRequestVar('search'), 'ip=') === 0) {
				...
				$condition .= ' AND (ip = '.$this->makeSafe($this->ipToStorageFormat($ip),'string').')';
				...
				$condition .= ' AND (userID = '.$this->makeSafe($userID,'integer').')';
			} else {
				// Use the search value as regular expression on message text and username:
				$condition .= ' AND (userName REGEXP '.$this->makeSafe($this->getRequestVar('search'),'string').' OR text REGEXP '.$this->makeSafe($this->getRequestVar('search'),'string').')';
			}
		}
	...
	function addInvitation($userID, $channelID=null) {
		...
		$sql = 'INSERT INTO `prefix_ajax_chat_invitations` (
					userID,
					channel,
					dateTime
				)
	...
	function removeInvitation($userID, $channelID=null) {
		...
		$sql = 'DELETE FROM
				...
				WHERE
					userID='.$this->makeSafe($userID,'integer').'
					AND
					channel='.$this->makeSafe($channelID,'integer').';';
