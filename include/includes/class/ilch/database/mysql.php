...
class Ilch_Database_Mysql
{
    ...
    public function query($sql)
    {
        if (preg_match("/^UPDATE `?prefix_\S+`?\s+SET/is", $sql)) {
            $sql = preg_replace("/^UPDATE `?prefix_(\S+?)`?([\s\.,]|$)/i", "UPDATE `" . $this->prefix . "\\1`\\2", $sql);
        } elseif (preg_match("/^INSERT INTO `?prefix_\S+`?\s+[a-z0-9\s,\)\(]*?VALUES/is", $sql)) {
            $sql = preg_replace("/^INSERT INTO `?prefix_(\S+?)`?([\s\.,]|$)/i", "INSERT INTO `" . $this->prefix . "\\1`\\2", $sql);
        } else {
            $sql = preg_replace("/prefix_(\S+?)([\s\.,]|$)/", $this->prefix . "\\1\\2", $sql);
        }
