...
switch ($aid) {
    default : 
		...
		if ($aid == 'edit') {
			...
			$tpl->set_ar_out (Array ( 'aname' => 'Eintrag bearbeiten', 'task' => $task, 'file' => $row['file'], 'description' => $row['description'], ), 5) ;
		}else{
			$task = getTasks( '' );
			$tpl->set_ar_out (Array ( 'aname' => 'Eintrag hinzuf&uuml;gen', 'task' => $task, 'file' => '', 'description' => '', ), 5) ;
		}
