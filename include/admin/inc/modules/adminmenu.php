...
switch ($aid) {
    default : 
		...
		if ($aid == 'edit') {
			...
			$tpl->set_ar_out (Array ( 'aname' => 'Eintrag bearbeiten', 'modul' => $modul, 'kat' => $kat, 'pos' => $row['pos'] ), 5) ;
		}else{
			$kat = getKats ();
			$modul = dblistee ('' , 'SELECT `id`, `name` FROM `prefix_modules` WHERE `menu` = "" AND (`gshow` = 1 OR `ashow` = 1) ORDER BY `name` ASC');
			$tpl->set_ar_out (Array ( 'aname' => 'Eintrag hinzuf&uuml;gen', 'modul' => $modul, 'kat' => $kat, 'pos' => $row['pos'] ), 5) ;
		}
