...
if ($menu->get(1) == "confirm" AND isset($_GET['check'])) {
    ...
    if (db_num_rows($erg) == 1) {
        ...
        switch ($row['ak']) {
            // confirm regist
            case 1:
                if (0 == db_count_query("SELECT COUNT(*) FROM `prefix_user` WHERE `name_clean` = BINARY '" . get_lower($row['name']) . "'")) {
                    db_query("INSERT INTO `prefix_user` (`name`,`name_clean`,`pass`,`recht`,`regist`,`llogin`,`email`,`status`,`opt_mail`,`opt_pm`)
  			 VALUES('" . $row['name'] . "','" . get_lower($row['name']) . "','" . $row['pass'] . "',-1,'" . time() . "','" . time() . "','" . $row['email'] . "',1,1,1)");
            ...
            // join us
            case 4:
                echo '<br />Joinus kann über diese Liste nicht akzeptiert werden, mache diese über <a style="color:red;" href="admin.php?groups-joinus">Joinus Anfragen bearbeiten</a><br /><br />';
                break;
            // ak 5 remove account
            case 5:
