                    ...
                    $(document).ready(function() {
                        $( "#akdialog" ).dialog({
                            ...
                            buttons: {
                                Ok: function() {
                                    $( this ).dialog( "close" );
                                    window.location = "admin.php?wars-gametype";
                                }
                            }
                        });
                    });
...
if (isset($_POST['newsubmit']) and chk_antispam('adminuser_action', true)) {
    ...
    $newgametypename		= escape($_POST['newgametypename'], 'string');
    $newinaktive		= escape($_POST['newinaktive'], 'integer');
    $qry 	=	db_query("INSERT INTO `prefix_wars_gametype` (name,inaktive) VALUES ('".$newgametypename."',".$newinaktive.")");
    if($qry) {
...
if ( $statusänderung == 'stat' AND !empty($statusänderungsid)) {
    $state=db_result(db_query('SELECT inaktive FROM `prefix_wars_gametype` WHERE id = '.$statusänderungsid),0);
    if($state==1){
        debug('########## Spiel aktivieren ##########');
        $reakqyr = db_query('UPDATE `prefix_wars_gametype` SET inaktive=0 WHERE id = "'.$statusänderungsid.'" LIMIT 1');
        if($reakqyr){
            echo $aktiviert;
        }
    } else {
         debug('########## Spiel deaktivieren ##########');
         $deakqyr = db_query('UPDATE `prefix_wars_gametype` SET inaktive=1 WHERE id = "'.$statusänderungsid.'" LIMIT 1');
        if($deakqyr){
            echo $deaktiviert;
        }
    }
...
switch ($seite) {
    case 'e':
		if($getid != 0 and !empty($getid)) {
			$editqry = db_query("SELECT * FROM `prefix_wars_gametype` WHERE id = ".$getid."");
			$outar = db_fetch_assoc($editqry);
				...
				if (isset($_POST['editsubmit']) and chk_antispam('adminuser_action', true)) {
						$editgametypename			= @escape($_POST['editgametypename'], 'string');
						$editinaktive			= escape($_POST['editinaktive'], 'integer');
						// DB UPDATE
					db_query("UPDATE `prefix_wars_gametype` SET
												name 	= '".$editgametypename."',
												inaktive 	= '".$editinaktive."'
											WHERE
												id = ".$getid."");
							wd('admin.php?wars-gametype', 'Daten gespeichert', 3);
							$design->footer(1);
			}
	...
	default:
        ...
        $class='Cmite';
		$oneout['siteindex']	= db_make_sites ($page ,$WHERE ,$limit ,'admin.php?wars-gametype' ,'wars-gametype');
        $oneout['antispam'] = get_antispam('adminuser_action', 0, true);
		$tpl->set_ar_out($oneout, 0);
		$erg=db_query("SELECT * FROM `prefix_wars_gametype` LIMIT ".$anfang.",".$limit);
