...
if ( $statusChange == 'stat' AND !empty($statusChangeid)) {
    $state=db_result(db_query('SELECT inaktive FROM `prefix_wars_games` WHERE id = '.$statusChangeid),0);
    if($state==1){
        debug('########## Spiel aktivieren ##########');
        $reakqyr = db_query('UPDATE `prefix_wars_games` SET inaktive=0 WHERE id = "'.$statusChangeid.'" LIMIT 1');
        if($reakqyr){
            echo $aktiviert;
        }
    } else {
         debug('########## Spiel deaktivieren ##########');
         $deakqyr = db_query('UPDATE `prefix_wars_games` SET inaktive=1 WHERE id = "'.$statusChangeid.'" LIMIT 1');
        if($deakqyr){
            echo $deaktiviert;
        }
    }
...
switch ($seite) {
	...
	default:
        ...
        $class='Cmite';
        $oneout['siteindex']	= db_make_sites ($page ,$WHERE ,$limit ,'admin.php?wars-games' ,'wars_games');
