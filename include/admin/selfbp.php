...
function save_file_to ($filename, $data, $flags = 0, $f = false) {
    ...
    else if ($f === false) $f = fopen($filename, 'w');
    if (round($flags / 2) == 1) while (!flock($f, LOCK_EX)) {
        /* lock */ }
...
$tpl->set_ar_out(array('akl' => $akl, 'text' => $text, 'filename' => $filename, 'exfilename' => $filename, 'wysiwyg' => $properties['wysiwyg'],
        ...
        'wysiwyg_editor' => $properties['wysiwyg'] == 1 ? '<script type="text/javascript">buttonPath = "include/images/icons/editor/"; imageBrowse = "admin.php?selfbp-imagebrowser"; makeWhizzyWig("bbwy", "all");</script>' : '') , 0);
$design->footer();
