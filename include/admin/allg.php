...
if (empty($_POST['submit'])) {
    ...
    while ($row = db_fetch_assoc($erg)) {
        ...
        if ($katname != $row['kat']) {
            $katname = $row['kat'];
            $katid = strtolower(preg_replace('%[^a-z0-9]%i', '', $katname));
            $tpl->set_ar_out(
                array(
                    'katid' => $katid,
                    'katname' => $katname
    ...
    if (isset($_POST['chosenTab'])) {
        $chosenTab = '#' . $_POST['chosenTab'];
    } else {
        $chosenTab = '';
    }
    wd('admin.php?allg' . $chosenTab, 'Erfolgreich ge&auml;ndert', 2);
}
// -----------------------------------------------------------|
$design->footer();
